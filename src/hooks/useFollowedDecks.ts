...
export const useFollowedDecks = () => {
  ...
  // Using a stable reference to the service functions
  const fetchFollowedDeckIdsFromServer = useCallback(() => {
    return followedDeckService.getFollowedDecks();
  }, []);
  
  const fetchFollowedDecksDataFromServer = useCallback(() => {
    return followedDeckService.getFollowedDecksData();
  }, []);
  
  // Fixed fetchFollowedDecks to avoid circular dependencies
  const fetchFollowedDecks = useCallback(() => {
    if (!isAuthenticated) {
      setFollowedDeckIds([]);
      setFollowedDecks([]);
      setIsLoading(false);
  ...
  const followDeck = useCallback((deckId: string) => {
    if (!isAuthenticated) {
  ...
  const unfollowDeck = useCallback((deckId: string) => {
